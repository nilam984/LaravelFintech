        ...
        {{-- <script>
            $(document).ready(function() {
                ...
                function validateCustomerStep() {
                    ...
                    let panImage = $('#pan_image')[0].files.length;

                    // Name
                    if (name === '') {
                        $('.error-name').text('Please enter customer name.');
                        valid = false;
                    }
                    ...
                    // Mobile
                    if (mobile === '') {
                        $('.error-mobile').text('Please enter mobile number.');
                        valid = false;
                    ...
                    // PAN
                    if (pan === '') {
                        $('.error-pan_no').text('Please enter PAN number.');
                        valid = false;
            ...
            `;
                ...
                function loadServices() {
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            ToastEngine.show(
                                xhr.responseJSON?.message || "Unable to load services.",
                                "error"
                            );
                        }
                    });
            ...
            $('#addUserForm').submit(function(e) {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        $('#nextStepBtn').prop('disabled', true).text('Checking...');
                    },
                    success: function(response) {
                        if (response.status) {
                            $('#userDetailsStep').addClass('hidden');
                            $('#serviceSelectionStep').removeClass('hidden');
                            loadServices();
                        } else {
                            ToastEngine.show(response.message, "error");
                        }
                    ...
                    complete: function() {
                        $('#nextStepBtn').prop('disabled', false).text('Continue');
                    }
                });
            });
            ...
            $('#payNowBtn').click(function() {
                ...
                if (selectedServices.length === 0) {
                    ToastEngine.show("Please select at least one service.", "error");
                    return;
                }
                $.ajax({
                    ...
                    error: function(xhr) {
                        ToastEngine.show(
                            xhr.responseJSON?.message || "Unable to initiate payment.",
                            "error"
                        );
                    }
                });
